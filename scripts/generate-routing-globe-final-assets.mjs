...
const demo = (theme, size, state) => {
  const label = state === "active" ? "Active" : state === "inactive" ? "Inactive" : "Reduced motion (active)";
  // Active routing deliberately keeps moving even when the OS requests
  // reduced motion, so this preview state uses the active sprite as well.
  const classes = ["globe", state === "inactive" ? "" : "globe--active"].filter(Boolean).join(" ");
