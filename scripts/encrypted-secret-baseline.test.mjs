...
assert.ok(
  migrationNames.includes("0010_encrypted_secret_baseline.sql"),
  "0010 encrypted-secret baseline migration must exist after confirming the migration number is free",
);

const migration = await readFile(
  "src-tauri/src/persistence/migrations/0010_encrypted_secret_baseline.sql",
  "utf8",
);
...
assert.ok(
  migration.includes("ALTER TABLE secrets ADD COLUMN key_id TEXT") &&
    ...
    !migration.includes("SET schema_version = 10"),
  "0010 should add transitional structure without directly activating the encrypted-secret profile",
);
...
assert.ok(
  !runtime.includes("crate::services::secrets") &&
    migrations.includes("readable_schema: 10..=10") &&
    migrations.includes("writable_schema: BTreeSet::from([10])"),
