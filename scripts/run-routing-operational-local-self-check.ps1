...
$commandPlan = @(
  ...
  [pscustomobject]@{ id = "persistence-baseline"; command = "cargo"; arguments = @("test", "--locked", "--manifest-path", $CargoManifest, "--lib", "persistence", "--", "--nocapture"); proves = @("schema-15 baseline conversion reaches the current encrypted schema", "conversion journals resume or fail closed", "fresh data stores publish a durable current database") },
