...
export function buildPaginationItems(page: number, totalPages: number): PaginationItem[] {
  const safeTotalPages = safePageNumber(totalPages);
  const safePage = Math.min(safePageNumber(page), safeTotalPages);
...
export function Pagination({ ariaLabel, page, totalPages, disabled = false, onPageChange }: PaginationProps) {
  ...
  return (
    <nav className="max-w-full overflow-x-auto" aria-label={ariaLabel}>
      <div className="flex min-w-max items-center">
        ...
        {items.map((item, index) => item === "ellipsis" ? (
          <span
            key={`ellipsis-${index}`}
            aria-hidden="true"
            className="-ml-px inline-flex h-8 min-w-9 shrink-0 items-center justify-center border border-border bg-surface px-2 text-muted-foreground"
          >
            ...
          </span>
        ) : (
          <button
            ...
            className={`-ml-px inline-flex h-8 min-w-9 shrink-0 items-center justify-center border px-2 text-sm transition-colors focus-visible:z-20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/30 disabled:cursor-not-allowed ${
              item === safePage
