...
export type MonitorProxyModeInputDto = "inherit" | "direct" | "system" | "manual";
...
export type CreateChannelMonitorInputDto = {
  ...
  intervalSeconds: number; proxyMode: MonitorProxyModeInputDto; proxyUrl: string | null;
...
export type ChannelStatusLatestResultDto = {
  ...
  httpStatus: number | null; latencyMs: number | null; ttfbMs: number | null;
  firstContentMs: number | null; finishedAtMs: number | null;
...
export type ChannelStatusRecentPointDto = {
  ...
  failureKind: string | null; terminalReason: string | null;
  httpStatus: number | null; latencyMs: number | null; ttfbMs: number | null;
  firstContentMs: number | null; checkedAtMs: number | null;
...
export type ChannelMonitorLatestSummaryDto = {
  monitorId: string; stationId: string; stationKeyId: string | null;
  latest: ChannelStatusLatestResultDto | null; running: ChannelStatusRunningExecutionDto | null;
};
...
export type ChannelMonitorTargetResultRecordDto = {
  ...
  trafficEquivalence: string; availabilityEligible: boolean; latencyEligible: boolean;
  exclusionReason: string | null; technicalHealthEffect: string;
  healthPolicyMode: string; healthWritebackDecision: string;
  healthWritebackReason: string | null; latencyMs: number | null; ttfbMs: number | null;
  firstContentMs: number | null; semanticConfidence: string;
...
export type ChannelMonitorAttemptRecordDto = {
  ...
  latencyMs: number | null; ttfbMs: number | null; firstContentMs: number | null;
  httpStatus: number | null; outcome: string; failureKind: string | null;
...
export type ChannelMonitorDto = {
  ...
  proxyMode: MonitorProxyModeInputDto; proxyUrl: string | null;
...
export type ModelPriceSyncStateDto = {
  sourceUrl: string; autoSyncEnabled: boolean; includeCommonModels: boolean;
  selectedModelKeys: string[]; excludedCommonModelKeys: string[];
  lastSyncAt: string | null; lastSyncError: string | null; modelCount: number; commonModelCount: number;
  autoSyncModelCount: number; filePath: string;
};

export type ModelPriceCatalogEntryDto = {
  key: string; provider: string; model: string; name: string; common: boolean;
  releaseDate: string | null; inputPrice: number | null; outputPrice: number | null;
  cacheCreationPrice: number | null; cacheReadPrice: number | null;
};

export type ModelPriceSyncResultDto = {
  state: ModelPriceSyncStateDto; importedCount: number; skippedCount: number;
};
...
export type ModelBasePriceIdInputDto = { id: string };

export type SaveModelPriceSyncConfigInputDto = {
...
export type SyncModelPricesInputDto = { force: boolean };
...
export type RoutingPlannerEvaluationStatusDto = "available" | "unavailable";
export type RoutingScoreStatusDto = "scored" | "excluded" | "candidate_limit" | "unavailable";
export type RoutingAvailabilityStatusDto = "available" | "capacity_exhausted" | "capacity_state_unavailable" | "all_keys_unavailable";
export type RoutingCandidateCapacityStatusDto = "available" | "exhausted" | "state_unavailable" | "unknown";
export type RoutingCandidateParticipationStatusDto = "eligible" | "conditionally_eligible" | "excluded" | "unavailable";
export type RoutingCandidateParticipationReasonDto = "ready" | "administratively_disabled" | "planner_excluded" | "planner_unavailable" | "candidate_limit_exceeded" | "circuit_persistence_unavailable" | "circuit_open_cooldown" | "circuit_recovery_score_gate_passed" | "circuit_recovery_score_gate_denied" | "circuit_half_open_idle" | "circuit_half_open_lease_occupied";
export type CircuitReadModelStatusDto = "available" | "unavailable";
export type CircuitPersistenceStatusDto = "available" | "unavailable";
export type CircuitReadStateDto = "closed" | "open" | "half_open";
export type RoutingMonitoringSourceStatusDto = "comparable" | "no_evidence" | "incomparable" | "weight_zero" | "disabled";
...
export type RequestDecisionDetailAvailabilityDto = "detailed" | "summary_only" | "unavailable";
// The last three values are read-only compatibility for pre-v3 trace history.
export type RequestDecisionActionDto = "retry_current_key" | "try_next_key" | "stop_request" | "retry_same_target" | "wait_then_replan" | "try_different_failure_domain";
...
export type RequestDecisionTimelineItemDto = {
  ...
  summary: string;
  detailCode: string;
  detailAvailability: RequestDecisionDetailAvailabilityDto;
  explanationKey: string | null;
  action: RequestDecisionActionDto | null;
  attemptOrdinal: number | null;
  remainingAttempts: number | null;
  remainingPrecommitBudgetMs: number | null;
...
export type RoutingCandidateCapacitySnapshotDto = {
  ...
  status: RoutingCandidateCapacityStatusDto;
...
export type RoutingCandidateCircuitStateDto = "closed" | "open" | "half_open";
export type RoutingCandidateScoreGateStatusDto = "not_applicable" | "waiting_cooldown" | "passed" | "denied" | "unavailable";

export type RoutingCandidateQualityWindowDiagnosticsDto = {
  sampleCount: number;
...
export type RoutingCandidateQualitySourceDiagnosticsDto = {
  eligible: boolean;
  effectiveWeightBasisPoints: number;
  recent: RoutingCandidateQualityWindowDiagnosticsDto;
  historical: RoutingCandidateQualityWindowDiagnosticsDto;
  blendedReliabilityBasisPoints: number;
};

export type RoutingCandidateLatencyDiagnosticsDto = {
  recentSampleCount: number;
  recentEffectiveWeight: number;
  recentWeightedLatencyMs: number;
  recentMinimumMet: boolean;
  historicalSampleCount: number;
  historicalEffectiveWeight: number;
  historicalWeightedLatencyMs: number;
  historicalMinimumMet: boolean;
  blendedWeightedLatencyMs: number;
};

export type RoutingCandidateQualityDiagnosticsDto = {
  qualityRevision: number;
  qualityPolicyRevision: number;
  algorithmVersion: string;
  qualityBasis: string;
  qualityUnavailable: boolean;
  canonicalSampleCount: number;
  realReliabilityBasisPoints: number;
  monitoringReliabilityBasisPoints: number;
  effectiveRealWeightBasisPoints: number;
  effectiveMonitoringWeightBasisPoints: number;
  realSourceEligible: boolean;
  monitoringSourceEligible: boolean;
  monitoringSourceStatus: RoutingMonitoringSourceStatusDto;
  recentSampleCount: number;
  recentEffectiveMassBasisPoints: number;
  recentMinimumSamples: number;
  historicalSampleCount: number;
  historicalEffectiveMassBasisPoints: number;
  historicalMinimumSamples: number;
  realSource: RoutingCandidateQualitySourceDiagnosticsDto;
  monitoringSource: RoutingCandidateQualitySourceDiagnosticsDto;
  latency: RoutingCandidateLatencyDiagnosticsDto;
  idleRealRouteSample: string;
  lastRealRouteSampleAtMs: number | null;
};

export type RoutingCandidateAttemptDiagnosticsDto = {
  rawRealAttemptCount: number;
  deduplicatedRealRequestCount: number;
};

export type RoutingCandidateCircuitDiagnosticsDto = {
  state: RoutingCandidateCircuitStateDto;
  stateRevision: number | null;
  lifecycleRevision: number | null;
  policyRevision: number | null;
  persistenceStatus: CircuitPersistenceStatusDto;
  stateRowPresent: boolean;
  consecutiveFailures: number | null;
  reopenLevel: number;
  cooldownUntilMs: number | null;
  cooldownRemainingMs: number | null;
  halfOpenLeaseInFlight: boolean;
  halfOpenLeaseExpiresAtMs: number | null;
  recoverySuccesses: number | null;
  scoreGateStatus: RoutingCandidateScoreGateStatusDto;
  scoreGateReason: string;
  bestClosedEffectiveScore: number | null;
};

export type RoutingCandidateDiagnosticsDto = {
  effectiveScore: number | null;
  baseScore: number | null;
  quality: RoutingCandidateQualityDiagnosticsDto | null;
  attempts: RoutingCandidateAttemptDiagnosticsDto;
  circuit: RoutingCandidateCircuitDiagnosticsDto;
};
...
export type RoutingWorkspaceCandidateDto = {
  ...
  healthState: string;
  score: number | null;
  scoreStatus: RoutingScoreStatusDto;
  participationStatus: RoutingCandidateParticipationStatusDto;
  participationReason: RoutingCandidateParticipationReasonDto;
  ...
  diagnostics?: RoutingCandidateDiagnosticsDto | null;
...
export type RoutingCandidateScoreWindowSnapshotDto = {
  ...
  recentRealSampleCount: number;
  recentMonitoringSampleCount: number;
  recentEffectiveMassBasisPoints: number;
  recentSuccessMassBasisPoints: number;
  recentFailureMassBasisPoints: number;
  recentMinimumSamples: number;
  recentReliabilityMinimumMet: boolean;
  recentScore: number;
  recentWeightBasisPoints: number;
  recentResponsivenessWeightBasisPoints: number;
  recentLatencySampleCount: number;
  recentLatencyEffectiveMassBasisPoints: number;
  recentWeightedLatencyMs: number;
  recentLatencyMinimumMet: boolean;
  recentRealLatencySampleCount: number;
  recentMonitoringLatencySampleCount: number;
  recentRealWeightedLatencyMs: number;
  recentMonitoringWeightedLatencyMs: number;
  recentRealLatencyMinimumMet: boolean;
  recentMonitoringLatencyMinimumMet: boolean;
  responsivenessRealSourceWeightBasisPoints: number;
  responsivenessMonitoringSourceWeightBasisPoints: number;
  recentLatencyCoverageBasisPoints: number;
  recentResponsivenessBasisPoints: number;
  historicalObservationCount: number;
  historicalRealSampleCount: number;
  historicalMonitoringSampleCount: number;
  historicalEffectiveMassBasisPoints: number;
  historicalSuccessMassBasisPoints: number;
  historicalFailureMassBasisPoints: number;
  historicalMinimumSamples: number;
  historicalReliabilityMinimumMet: boolean;
  historicalScore: number;
  historicalWeightBasisPoints: number;
  historicalResponsivenessWeightBasisPoints: number;
  historicalLatencySampleCount: number;
  historicalLatencyEffectiveMassBasisPoints: number;
  historicalWeightedLatencyMs: number;
  historicalLatencyMinimumMet: boolean;
  historicalRealLatencySampleCount: number;
  historicalMonitoringLatencySampleCount: number;
  historicalRealWeightedLatencyMs: number;
  historicalMonitoringWeightedLatencyMs: number;
  historicalRealLatencyMinimumMet: boolean;
  historicalMonitoringLatencyMinimumMet: boolean;
...
export type RoutingWorkspaceAggregatesDto = {
  totalCandidates: number;
  schedulableCandidates: number;
  eligibleCandidates: number;
  conditionallyEligibleCandidates: number;
  excludedCandidates: number;
  unavailableCandidates: number;
  closedCircuits: number;
  openCircuits: number;
  halfOpenCircuits: number;
  persistenceUnavailableCircuits: number;
};

export type CircuitReadSnapshotRevisionDto = {
  processGateRevision: number;
  persistenceHealthRevision: number;
  stateFingerprint: string;
};

export type StationKeyCircuitReadFactDto = {
  stationKeyId: string;
  lifecycleRevision: number;
  state: CircuitReadStateDto;
  stateRevision: number | null;
  policyRevision: number | null;
...
export type RoutingCircuitStatusDto = {
  statusVersion: string;
  sourceSchemaVersion: number;
  generatedAtMs: number;
  readModelStatus: CircuitReadModelStatusDto;
  readModelCode: string | null;
  revision: CircuitReadSnapshotRevisionDto;
  circuits: StationKeyCircuitReadFactDto[];
};

export type RoutingWorkspaceSnapshotDto = {
  readModelVersion: string;
  generatedAtMs: number;
  policyConfig: RoutingPolicyConfigV3Dto;
  previewPolicyVersion: string;
  maxRateMultiplier: number | null;
  routingGroupFilter: RoutingGroupFilterDto;
  ...
  plannerEvaluation: RoutingPlannerEvaluationStatusDto;
  plannerEvaluationCode: string | null;
  availabilityStatus: RoutingAvailabilityStatusDto;
  aggregates: RoutingWorkspaceAggregatesDto;
  circuitReadModelStatus: CircuitReadModelStatusDto;
  circuitReadModelCode: string | null;
  circuitRevision: CircuitReadSnapshotRevisionDto;
  runtimeGenerationId?: string | null;
  policyRevision?: number | null;
  qualityRevision?: number | null;
  healthRevision?: number | null;
  qualityProjectionBacklog?: number | null;
  qualityProjectionLagSeconds?: number | null;
  qualityStale?: boolean | null;
};
...
export type ProtectionPersistenceKindDto = "durable" | "legacy_compatibility" | "runtime_capacity";
export type ProtectionStateDto = "no_protection" | "degraded" | "cooldown" | "blocked" | "open" | "half_open" | "unavailable";
export type ProtectionReadModelStatusDto = "available" | "unavailable";
export type ProtectionDiagnosticReasonDto = "capacity_exhausted" | "capacity_state_unavailable";
export type ProtectionStatusEntryDto = {
  scope: string;
  scopeKind: string | null;
  state: ProtectionStateDto;
  explanationKey: string;
  persistenceKind: ProtectionPersistenceKindDto | null;
  cooldownUntilMs: number | null;
  cooldownRemainingMs: number | null;
  recentFailureCode: string | null;
  diagnosticReason: ProtectionDiagnosticReasonDto | null;
  updatedAtMs: number | null;
  detailAvailable: boolean;
};
export type RoutingProtectionStatusDto = {
  statusVersion: string;
  generatedAtMs: number;
  entries: ProtectionStatusEntryDto[];
  readModelStatus: ProtectionReadModelStatusDto;
  timeouts: ProxyTimeoutFactsDto | null;
};

export type RoutingProtectionStatusInputDto = Record<string, never>;
export type ProxyTimeoutFactsDto = {
  connectSeconds: number;
  firstByteSeconds: number;
  precommitSeconds: number;
  bufferedExecutionSeconds: number;
...
export type RetryFailoverPolicyV2Dto = {
  version: number;
  maxTotalAttempts: number;
  maxSameTargetCapacityRetries: number;
  capacityRetryWaitBudgetSeconds: number;
  allowCrossCapacityDomainFallback: boolean;
};

export type ProtectionProfileConfigV2Dto = {
  version: number;
  enabled: boolean;
...
export type TimeoutPolicyV2Dto = {
  version: number;
  connectSeconds: number;
  firstByteSeconds: number;
  precommitSeconds: number;
...
export type RoutingPolicyConfigV2Dto = {
  version: number;
  reliabilityWeight: number;
  responsivenessWeight: number;
  costWeight: number;
  ...
  outboundProxyMode: string;
  outboundProxyUrl: string | null;
  retryFailover: RetryFailoverPolicyV2Dto;
  protectionProfile: ProtectionProfileConfigV2Dto;
  timeoutPolicy: TimeoutPolicyV2Dto;
};

export type ReliabilitySourceWeightsV3Dto = {
  realTrafficPercent: number;
  monitoringPercent: number;
};

export type ReliabilitySamplingPolicyV3Dto = {
  historicalMinimumSamples: number;
  recentMinimumSamples: number;
  optimisticReliabilityPercent: number;
  optimisticLatencyMs: number;
};

export type RetryPolicyV3Dto = {
  version: number;
  maxRetryCount: number;
  consecutiveFailureThreshold: number;
};

export type CircuitBreakerPolicyV3Dto = {
  version: number;
  recoverySuccessThreshold: number;
  recoveryWaitSeconds: number;
};

export type RoutingPolicyConfigV3Dto = {
  version: number;
  reliabilityWeight: number;
  responsivenessWeight: number;
  costWeight: number;
  ...
  outboundProxyMode: string;
  outboundProxyUrl: string | null;
  reliabilitySourceWeights: ReliabilitySourceWeightsV3Dto;
  reliabilitySampling: ReliabilitySamplingPolicyV3Dto;
  retry: RetryPolicyV3Dto;
  circuitBreaker: CircuitBreakerPolicyV3Dto;
  timeoutPolicy: TimeoutPolicyV2Dto;
};
...
export type ApplyRoutingPolicyDocumentInputDto = {
  ...
  baseRevision: number;
  policy: RoutingPolicyConfigV3Dto;
};

export type RoutingPolicyPublicationStateDto =
  | "staged"
  | "ready"
...
export type RoutingPolicyActivationPathDto = "fast" | "generation" | "persisted_only";

export type RoutingPolicyFallbackReasonDto =
  | "quality_tail"
  | "circuit_tail"
  | "concurrent_revision"
  | "runtime_unavailable"
...
export type RoutingPolicyPublicationFailureCodeDto =
...
export type RoutingPolicySnapshotDto = {
  config: RoutingPolicyConfigV3Dto;
  revision: number;
  policyVersion: string;
  systemVersion: string;
  status: string;
  activationPath?: RoutingPolicyActivationPathDto | null;
  runtimeStatus?: RoutingPolicyPublicationStateDto | null;
  activeRevision?: number | null;
  fallbackReason?: RoutingPolicyFallbackReasonDto | null;
  updatedAtMs: number;
  documentSync: RoutingDocumentSyncDto | null;
};

export type RoutingPolicyPublicationStatusInputDto = {
  revision: number;
  policyGenerationId?: string | null;
};

export type RoutingPolicyPublicationStatusDto = {
  revision: number;
  policyGenerationId: string | null;
  status: RoutingPolicyPublicationStateDto;
  failureCode: RoutingPolicyPublicationFailureCodeDto | null;
  activationPath?: RoutingPolicyActivationPathDto | null;
  runtimeStatus?: RoutingPolicyPublicationStateDto | null;
  activeRevision?: number | null;
  fallbackReason?: RoutingPolicyFallbackReasonDto | null;
...
export type RoutingDocumentSyncDto = {
  state: "synchronized" | "pending_materialization" | "external_change" | "error";
...
export type RedeemStationCodeInputDto = { stationId: string; code: string };
export type StationRedemptionResultDto = { provider: string; success: boolean; message: string; creditedDetail: string | null };
...
export type MutationRevisionDto = { scope: string; revision: number };

export type MutationReceiptDto = {
  mutationId: string;
  committedAtMs: number;
  affectedScopes: string[];
  revisionVector: MutationRevisionDto[];
};

export type CollectorRunResultDto = {
  snapshot: CollectorSnapshotDto; events: CollectorEventDto[]; receipt: MutationReceiptDto;
};
