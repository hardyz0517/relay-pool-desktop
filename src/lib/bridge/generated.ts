...
export type ErrorRateHistoryInputDto = { beforeMs?: number | null; limit?: number | null };
...
export type RequestDecisionActionDto = "retry_same_target" | "wait_then_replan" | "try_different_failure_domain" | "stop_request";
...
export type HealthProtectionScopeKindDto = "credential" | "account" | "endpoint" | "model" | "capacity_domain";
export type HealthProtectionFailureCodeDto = "connect_failure" | "first_byte_timeout" | "upstream_5xx" | "rate_limited" | "capacity_exhausted" | "endpoint_unavailable" | "unknown";
export type ErrorRateHistoryOutcomeDto = "success" | "failure";
export type HealthProtectionTransitionCodeDto = "ignored_duplicate" | "observed" | "opened" | "probe_succeeded" | "closed" | "reopened";
export type ErrorRateHistoryEventDto = {
  observedAtMs: number;
  scopeKind: HealthProtectionScopeKindDto;
  scopeCommitment: string;
  outcome: ErrorRateHistoryOutcomeDto;
  failureCode: HealthProtectionFailureCodeDto | null;
  sampleCount: number;
  failureCount: number;
  failureRatePercent: number;
  transition: HealthProtectionTransitionCodeDto | null;
};
export type ErrorRateHistoryPageDto = {
  version: string;
  enabled: boolean;
  detailAvailable: boolean;
  events: ErrorRateHistoryEventDto[];
...
export type RequestDecisionTimelineItemDto = {
  ...
  remainingWaitBudgetMs: number | null;
...
export type RoutingCandidateFailureDomainSnapshotDto = {
  kind: "capacity_domain";
  resolution: "not_configured" | "invalid_identity" | "model_required" | "resolved";
...
export type RoutingWorkspaceCandidateDto = {
  ...
  failureDomain: RoutingCandidateFailureDomainSnapshotDto;
...
export type RoutingWorkspaceSnapshotDto = {
  ...
  generatedAtMs: number;
  policyConfig: RoutingPolicyConfigV2Dto;
  previewPolicyVersion: string;
  maxRateMultiplier: number | null;
  routingGroupFilter: RoutingGroupFilterDto;
...
export type RoutingFailureDomainDiagnosticDto = {
  commitment: string | null;
  resolution: "not_configured" | "invalid_identity" | "model_required" | "resolved";
  providerFamily: string | null;
  deploymentIdentity: string | null;
...
export type RoutingProtectionStatusDto = {
  ...
  failureDomains?: RoutingFailureDomainDiagnosticDto[];
  readModelStatus: ProtectionReadModelStatusDto;
  timeouts: ProxyTimeoutFactsDto | null;
};

export type RoutingProtectionStatusInputDto = {
  model?: string | null;
};
...
export type ApplyRoutingPolicyDocumentInputDto = {
  ...
  baseRevision: number;
  policy: RoutingPolicyConfigV2Dto;
};

export type RoutingPolicySnapshotDto = {
  config: RoutingPolicyConfigV2Dto;
  revision: number;
