...
状态：Ready for implementation；本文只处理路由工作区中“可展示候选、规划准入、评分状态”口径不一致的问题，不改路由策略或兑换率公式。
...
> 本计划修复的是事实与解释的断裂，不是放宽任何资格门。当前价格模型以 `model_base_prices`、`station_group_bindings.effective_rate_multiplier` 和站点 `creditPerCny` 为来源，不再存在 `pricing_rules` 作为价格或倍率 fallback。`0.07x` 等实际倍率仅是成本评分输入；候选是否进入评分仍由资格、分层和候选上限决定。
...
PlanningCandidateAssessment
  ...
  candidate_set: within_limit | capped_by_candidate_limit
  primary_reason: stable enum/code (required unless admitted_for_scoring)
...
`probe_discovery_only` 是当前 error-rate protection 为恢复 Half-Open probe 保留的执行协调候选，不是用户目标评分候选。它即使暂时存在于 `PlanningSnapshot.candidates` 中，也不得获得 Workspace score 或被计入“可参与/已评分”统计；页面只说明“仅用于恢复探测”。`candidate_set` 与 `eligibility` 分离，避免把“因上限未进入本轮集合”错误说成业务资格失败，并能正确表达被上限截断的 probe discovery 候选。

`request-context fingerprint` 至少包含 route kind、是否为基线工作区请求、请求模型/映射输入是否存在、stream/tools/vision/reasoning、分组/标签和相关 policy revision。首期 Workspace 明确只展示“基线请求”的 assessment，不能暗示一个 key 对所有实际模型请求都可路由；未来增加模型筛选时以新的 context 生成 assessment，而不是复用基线结果。
...
| `probe_discovery` | 必为空 | 仅可用于受保护候选的恢复探测，不参与用户目标评分。 |
| `unavailable` | 必为空 | 仅当 snapshot 的 `planner_evaluation=unavailable`；不是业务排除。 |

`planner_exclusion_codes` 由 `primary_reason` 加安全的 secondary codes 投影而来，前端优先解释 primary reason。当前 `hard_rejection_codes` 不能整体改名后继续当作第二个 owner：只有与 assessment 逐项等价的规则才能迁入。其余展示事实必须另起字段并在名称中表明其只读性质，不能再影响 `previewEligible`。
...
模型基础价格的同步、恢复、增删改，站点 `creditPerCny` 更新，以及分组 binding 的倍率/状态更新成功后必须复用 `refreshRoutingQueries` 使 `routingQueryKeys.all` 失效；仍保留各页面已有的 pricing、station、key-pool、channel 失效，不以路由刷新替代它们。当前无 `pricing_rules` mutation，因此不得为本计划恢复或引用已删除的 pricing rule IPC/API。
...
1. 枚举 builder 当前所有提前返回和过滤点：模型映射无 offering/target、unsupported model、credential、协议/模型/feature、标签、分组、余额、scoped health、错误率保护和 `max_candidates`。同时逐项审计 Workspace 当前的 `hard_rejection_codes`，特别是 `multiplier_ceiling`、pricing basis 和 capacity；不能假定两者已经等价。
...
**目标：** 让“是否进入评分”及其理由只计算一次，并与实际 `PlanningSnapshot.candidates` 完全一致。
...
1. 先扩展 `RawOperationalCandidateRow` / `OperationalCandidateFact`，以强类型携带 eligibility 所需的站点 `credit_per_cny`、绑定原始倍率、binding status 和已有 revision；保持数据库列名兼容，但不把未归一化值在 Rust 领域模型中命名为 effective multiplier。不得从 Workspace 的 canonical DTO 反向取数，也不得逐候选读 pricing。
...
3. 对 model mapping 无结果、model-scoped verdict 和 error-rate admission 保留精确的排除理由，不能在中间 `return None` 时丢失原因。无法继续判断的短路分支只发出一个 primary reason；只有已安全获得的独立原因才作为 ordered secondary code。
4. 维持现有 health probe discovery 的执行语义，并在 assessment 中写为 `probe_discovery_only`。它可以保留在 `PlanningSnapshot.candidates` 供执行协调使用，但不获得 Workspace score，也不得伪装成可评分候选。
5. 在现有 inclusion predicate 之后，按原有确定性顺序应用 `max_candidates`；将 `candidate_set` 标记为 `within_limit` 或 `capped_by_candidate_limit`。被截断的 candidate 不得进入 `PlanningSnapshot.candidates` 或获得评分；其 underlying eligibility 必须保留以便解释。
6. 给 assessment 附带 station key、endpoint/credential/account/group revision、planning snapshot id 和 request-context fingerprint，以防将旧评估或基线评估回填给修改后的 key/不同请求。
7. 保持 builder 的批量读取和既有 source upper bound；模型基础价格仅在请求成本估算/展示需要时通过现有 `PricingStore::resolve_station_key_pricing_many` 批量解析。实际倍率资格和 score proxy 只消费 bundle 中的原始倍率 + `credit_per_cny` 归一化结果，禁止为这两项逐候选 pricing 查询。

**完成条件：** 任何 `scored` assessment 都对应一个 `hard_eligible=true` 的 planner candidate；每个普通 planner candidate 都有且只有一个 `admitted_for_scoring + within_limit` assessment；`probe_discovery_only` 可以留在 snapshot 但永不评分；候选 cap 与 health probe 行为保持原样；生产 `PlanningSnapshot` 的结构和热路径遍历范围不因诊断需求扩大。
...
1. 用 `assessment_by_key + request-context fingerprint` 替代只含 score 的 `score_by_key` join；评分 breakdown 只对 `admitted_for_scoring` assessment 计算。
2. 为每行填充 `score_status`、`planner_exclusion_codes` 和 assessment revision/snapshot provenance；保留现有分数载荷的精度与评分公式。`candidate_limit` 不是 hard rejection，不得混入 exclusion codes。
...
**完成条件：** `score == null` 不再是无解释状态；planner 失败、业务排除、候选截断在 DTO 上互不混淆；workspace 的“预览可用”来自 assessment 加其已声明的 baseline request context，而非本地猜测。
...
- Modify: `src/lib/query/pricingMonitoringInvalidation.ts` or mutation caller boundary
- Update: routing workspace view-model、状态行、诊断面板、query invalidation Vitest

**步骤**

1. 显示 `已评分`、`未进入评分：<原因>`、`候选上限外`、`评分暂不可用` 四类状态；score detail 弹窗仅在 `scored` 时可打开。
2. 用共享的前端 reason-code 文案表展示稳定中文说明；未知 code 回退为通用安全文案，不显示后端错误原文。
3. `previewEligible`、汇总计数和诊断面板改为消费 `planner_evaluation`、`score_status` 与 `planner_exclusion_codes`；`planner_evaluation=unavailable` 时明确显示“基线评估暂不可用”，不把所有候选计入可用或排除。
4. 所有会改变实际倍率/定价上下文的 mutation 完成后，统一调用 pricing 失效和 `refreshRoutingQueries`；测试证明 routing workspace 立即进入 refetch，而非等待五秒 stale window。
...
1. 只删除 Workspace 已由 assessment 覆盖的资格重算，特别是将 `hard_rejection_codes` 作为 `previewEligible` 来源的路径。先以 caller inventory 证明 `candidate_projector` 的同名代码没有独立 consumer；未证明前保持它不动。
...
4. 添加测试不变量：相同 baseline request context + durable revision 下，Workspace 行的 key/revision 集合与 assessment 集合一一对应，且 `scored` 行集合等于 `PlanningSnapshot.candidates` 集合；每个未评分行都有非空、稳定状态；planner error 或 source mismatch 不会伪造 `excluded`。
...
| 修改 `creditPerCny` 或定价规则 | refreshed workspace | 倍率、assessment provenance、评分来自同一有效版本。 |
...
本计划完成时，路由页面不再以孤立的 `—` 表示密钥评分。每个密钥都能明确显示“已评分”“被规划器排除及原因”“因候选上限未评分”或“基线评估暂不可用”；生产选择与 Workspace 解释复用同一资格核心，模拟在迁移时复用同一 assessment contract。兑换率修改后，实际倍率与评分状态在同一 durable read 中更新；除 Task 0 已按当前规范确认的遗漏资格门外，不改变既有资格与成本公式。
