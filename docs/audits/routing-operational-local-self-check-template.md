...
This checklist records the development-period reset/reimport/reconfigure proof for the current dev binary.

Runtime output belongs under ignored `output/routing-operational/self-check/local-self-check/` or CI artifacts.
...
- Real OpenAI-compatible client smoke:
  ...
  - Summary output defaults to ignored `output/routing-operational/self-check/local-client-smoke/` and records only redacted endpoint/database evidence plus request ids.
