...
状态：Approved and implemented；本文件是智能路由评分、重试、Key 熔断与恢复的当前 v3 实现规范。实现证据以代码、自动化契约和对应审计记录为准。
...
2. `最大重试次数`表示首把 Key 之外还可以尝试多少把不同 Key；默认 `3` 即最多尝试 4 把 Key。同一 Key 的普通失败由连续失败阈值约束，阈值内重复尝试当前 Key 不消耗这个数量。
...
- `RetryBudget`：单请求换 Key 预算，回答“首把 Key 之外，这次请求还可以尝试多少把不同 Key”。当前 Key 在熔断前的重复发送由连续失败阈值约束，不消耗该预算。
...
每次 outbound attempt 结束后，必须先写入本次 attempt 的结果并更新 durable circuit。普通可重试失败未使当前 Key Open 时，继续使用同一 Key，不触发全局配置重规划，也不加入 `request_excluded_keys`；达到连续失败阈值或 durable circuit 已因跨请求历史失败 Open 后，才排除当前 Key，并根据最新 runtime/durable revision 获取规划快照，按评分选择下一把 Key。未完成准入的容量拒绝不加入 `request_excluded_keys`，也不消耗可尝试 Key 名额。
...
7. 最后才按来源权重混合：
   ...
   默认配置为 `realTrafficWeight=0.7`、`monitoringWeight=0.3`。`effectiveRealTrafficWeight`/`effectiveMonitoringWeight` 由两类来源是否启用及其配置权重得到；来源权重只在这里应用一次，不根据样本数量再次动态调整。
8. 响应速度也必须使用相同的去重样本口径、窗口边界、`w_i`、`c` 和最小样本门槛。真实路由和主动监控先分别计算来源内的成功延迟，再按有效来源权重混合。令 `l_i` 为该独立请求或探针的有效响应时间（毫秒，必须是有限且非负数）：非流式请求使用完整响应时间，流式请求使用首字节时间（TTFT）。则计算：

   ```text
   L_{source,recent} = sum(w_i * l_i) / sum(w_i)       # n_{source,latency} >= recentMinimumSamples
   L_{source,history} = sum(w_i * l_i) / sum(w_i)      # m_{source,latency} >= historicalMinimumSamples
   L_{source,recent} = optimisticLatencyMs             # n_{source,latency} < recentMinimumSamples
   L_{source,history} = optimisticLatencyMs            # m_{source,latency} < historicalMinimumSamples
   L_source = c_{source,latency} * L_{source,recent}
            + (1 - c_{source,latency}) * L_{source,history}
   c_{source,latency} = min(0.9, n_{source,latency} / (n_{source,latency} + 20))
   L = effectiveRealTrafficWeight * L_real
     + effectiveMonitoringWeight * L_monitoring
   ```

   `n_{source,latency}`/`m_{source,latency}` 是对应来源、对应窗口内去重后拥有有效 `l_i` 且请求成功完成的独立样本数；它们可以少于该来源的可靠性样本数。失败、取消、缺失、负数或非有限延迟只从响应速度样本中排除，但失败仍可作为可靠性样本。每个来源先完成近期/历史聚合，再按有效来源权重混合成 `L`，最后才转换为现有的 `responsiveness_score`（延迟越低，速度分越高）；不能先把每条延迟转换成分数再做窗口或来源合成。转换使用代码常量 `RESPONSIVENESS_SCORE_CAP_MS=120_000`：先将 `L` 按 `latency_scale=1_000` half-up 量化为 `q_ms`，再计算 `responsiveness_score = floor(10_000 * (120_000 - min(q_ms, 120_000)) / 120_000)`，结果 clamp 到 `[0,10_000]`。来源权重沿用可靠性来源权重配置，且只在最终来源混合处使用一次，同时作用于可靠性和响应速度；不影响熔断连续失败计数。
...
`optimisticLatencyMs` 用于每个来源的最近/历史窗口样本不足时的响应时间输入，默认 `2_500ms`。响应速度严格使用第 6.2 节定义的 `w(t)`：真实路由和主动监控分别对成功样本的有效延迟按时间衰减加权，分别得到来源内的 `L_recent` 和 `L_history`，再用来源内的 `c_latency = min(0.9,n_latency/(n_latency+20))` 合成来源延迟，最后按有效真实流量/监控来源权重合成 `L`。因此这里的响应时间是来源加权后的加权平均毫秒值，不是未经定义的“延迟摘要”或自动 p95；后续若要改为 p95/分位数，必须另立算法版本和公式。失败延迟可以写入原始观测和诊断，但不能进入 `responsiveness_score`。长期保持高分的 Key 不需要被额外轮换，低分 Key 也不保证获得独立探测请求。`last_real_route_sample_at` 只接受具有有效 `event_at` 的真实路由 outcome；若最近事件缺少有效事件时间，状态页将 `idle_real_route_sample` 标为 `unknown`，不得把它误判为 24 小时内有样本或 24 小时闲置。
...
来源对状态效果的边界固定为：`RealRequest` 的可归责结果才生成 Key circuit event、推进连续失败或 Half-Open 恢复，以及驱动请求级 retry；`ActiveProbe` 只作为密钥级质量/诊断来源，不直接改变 Key circuit、不触发 request retry，也不加入 request exclusion。Half-Open 恢复必须来自带 lease revision 的真实路由请求。来源权重作用于可靠性和响应速度的最终质量混合。
...
可靠性评分的作用域是整把 `station_key_id`，不是某个模型、endpoint variant 或请求形态。一个密钥可以服务多个模型，因此 `ActiveProbe` 不再要求与某条 `RealRequest` 的 `comparability_key` 相同；只要监控观测属于当前密钥生命周期、已经跨过 outbound boundary、事件时间有效、请求集群已完成且不是匿名探测，就进入监控来源的独立样本池。`comparability_key` 仍可用于审计和诊断，但不能把模型不同的监控样本排除出密钥级评分。

`effective_*_weight_bps` 定义为：来源已启用且配置权重大于 0 时保留其配置权重，再将启用来源按比例归一化到总和 `10_000`。某来源暂时没有样本时仍保留其配置权重，并按第 6.2 节使用乐观值；只有没有任何正权重的启用来源时，才不生成可靠性分数并返回 `quality_unavailable` 诊断。质量详情必须分别展示实际流量样本、监控样本和最终混合结果。
...
| `401/403` 等凭据拒绝 | 失败 | 加一；必要时同时记录凭据诊断 | 安全时按普通错误重试当前 Key |
| 严格确认当前 Key 不支持请求模型 | 不计入 Key 可靠性；仅写能力诊断 | 不计入 | 直接结束，不重试也不换 Key |
| 严格确认余额不足 | 不计入 Key 可靠性；仅写账号诊断 | 不计入 | 直接结束，不重试也不换 Key |
| 上游普通 `400/409/422` 或请求拒绝 | 失败 | 加一 | 安全时按普通错误重试当前 Key |
| 下游在发送前取消 | 不计入 | 不计入 | 结束 |
...
`observed_reliability` 的可靠性为该来源窗口内 `sum(w_i * s_i) / sum(w_i)`；`weighted_latency` 为同一来源窗口内 `sum(w_i * l_i) / sum(w_i)`，其中 `l_i` 是有效端到端延迟。可靠性和响应速度都使用各自来源的 `n/m` 与 `c=min(0.9,n/(n+20))`，先应用来源内窗口最小样本门槛，再做近期/历史合成，最后按有效来源权重混合。最近窗口和历史窗口不重叠，时间衰减权重不得偷偷变成来源权重。
...
- 每个来源的 recent/history count 和 mass，以及是否达到对应门槛；真实路由和监控成功延迟的来源内 `n_latency/m_latency`、`L_recent/L_history/L_source`，以及最终来源混合后的 `L`；
...
计数跨请求持久化，并按每个可归责 outbound attempt 的 reducer 提交顺序累积（以配置的阈值 `N` 为准）。普通可重试失败发生后，只要该 Key 尚未达到阈值且请求仍可安全重放，就继续尝试当前 Key；同一请求可以为 circuit 贡献多次真实失败并在本请求内把它打开。质量统计仍按 `source + station_key_id + correlation_id` 去重，因此这些 raw attempt 不会把同一个客户端请求重复放大成多条可靠性样本。只有 Key 达到阈值并 Open 后，才把它加入本请求的 `request_excluded_keys` 并继续下一把 Key。

达到阈值时（首次打开的 `reopen_level` 固定为 `1`，不能保持为 `0`）：

1. 原子地写入 `Open`、`state_revision + 1`、`opened_at`、`cooldown_until` 和非敏感最近失败码。
2. 当前请求把该 Key 计作已经尝试的一把 Key，并在 `maxRetryCount` 允许时继续尝试其他可执行候选；不能因为打开保护就报告成功。
...
系统常量固定为：`MAX_OPERATIONAL_CANDIDATES=1024`、`QUALITY_PROJECTOR_BATCH_SIZE=256`、`MAX_PROJECTOR_BACKLOG=100_000`、`SYSTEM_RAW_ATTEMPT_HARD_CAP=40`、`SYSTEM_CUTOVER_FENCE_TIMEOUT_MS=30_000` 和 `HALF_OPEN_LEASE_REAPER_INTERVAL_MS=5_000`；这些常量不进入 policy v3 或设置页。
...
- `429` 与其他可重试的单 Key 故障完全同路径：每个真实失败都使连续失败计数加一；阈值内重试当前 Key，达到阈值后 Open，再按最新评分尝试下一把 Key。切换到下一把 Key 时才消耗一个 `maxRetryCount` 名额；不得因为 `Retry-After` 改走另一套等待或重试路径。
...
| 字段 | 中文标签 | 默认值 | 建议范围 | 语义 |
| --- | --- | ---: | ---: | --- |
| `maxRetryCount` | 最大重试次数 | 3 | `0..3` | 首把 Key 之外最多还可尝试的 Key 数量；最多尝试的不同 Key 数为 `1 + maxRetryCount` |
| `consecutiveFailureThreshold` | 连续失败阈值 | 3 | `1..10` | 当前 Key 的真实可归责失败达到该次数后 Open；阈值内继续重试当前 Key，计数跨请求持久化 |

`maxRetryCount=0` 表示本请求只使用评分最高的第一把 Key，但这把 Key 仍可在安全边界内重复尝试，直到成功、达到连续失败阈值、请求无法安全重放或请求超时；它不表示只发送一次，也不是关闭熔断器。失败阈值与可尝试 Key 数量必须是两个独立字段。

对普通 Closed Key，内部 raw outbound attempt 的最坏情况上限为：

```text
maxDistinctKeys = min(1 + maxRetryCount, eligibleDistinctKeyCount)
rawAttemptHardLimit = min(
  maxDistinctKeys * consecutiveFailureThreshold,
  SYSTEM_RAW_ATTEMPT_HARD_CAP
)
```

`SYSTEM_RAW_ATTEMPT_HARD_CAP` 是内部安全上限，不进入设置页。历史连续失败可能使某把 Key 少于本请求配置的阈值就 Open；Half-Open 也始终只放行一个真实请求，因此实际 raw attempt 通常低于该上限。同一 Key 的内部重试不消耗 `maxRetryCount`，只有开始尝试另一把不同 Key 才消耗一个名额。

### 9.2 可重试错误

除下述两个严格确认的业务终止错误外，普通上游错误统一进入同一条“重试当前 Key，达到阈值后再换 Key”的路径，包括：

- 上游返回的 `429`、rate limit、过载或容量不足错误（本地尚未发送的容量准入拒绝不属于此项）；
- 凭据拒绝、上游请求拒绝、普通 `400/409/422` 等错误；
- 已确认跨 outbound boundary 的连接/传输失败；
- 首字节前超时；
- 上游 `500..599`，包括上游 `502`；
- 已确认跨 outbound boundary、但具体语义未知的上游错误。

只有以下已严格确认的上游业务错误不重试当前 Key，也不切换其他 Key，直接返回：

- 当前 Key 不支持请求的模型；
- 当前账号或余额严格不足。

以下情况因请求安全或本地执行边界停止，不构成第三套用户可配置错误策略：
...
记录当前 raw attempt
  ...
  -> Key 尚未 Open：继续尝试当前 Key，不消耗 maxRetryCount
  -> Key 达到阈值 Open：排除当前 Key
  -> 尚有额外 Key 名额：按当前评分顺序选择下一把未尝试且可用的 Key
  -> 没有额外 Key 名额或没有可用 Key：按终态规则结束
```

同 Key 重试是普通失败的唯一默认重试方式，不是 429/容量错误专用旁路。达到阈值后，每次切换 Key 都从当前快照中尚未尝试且通过熔断/硬资格检查的候选按评分顺序遍历，再对每个候选依序申请容量准入；不能先按容量结果重排评分序列。容量准入成功后才发送；如果所有候选都无法准入，立即按终态规则结束请求。Half-Open 失败不在同一请求内重试当前 Key，而是立即重新 Open，以保持“同时只放行一个真实请求”的恢复契约。

本节的同 Key 重试语义适用于实际推理请求。`/models` 是只读聚合适配器，每把候选 Key 最多查询一次并合并结果，不使用同 Key 连续失败重试来重复拉取同一模型列表。
...
1. **评分偏好**
    ...
    - 增加“质量样本来源权重”：实际路由样本 / 监控样本，默认 70% / 30%；说明“先分别计算两类来源的可靠性和响应速度，再按这里的权重混合，只影响质量评分，不改变熔断连续失败计数”。
...
5. **会话亲和**
   - 保持当前开关和 TTL。
6. **重试设置**
    - **最大重试次数**：默认 `3`；说明“首把 Key 之外最多再尝试多少把 Key；同一 Key 内部重试不占用这个数量”。
    - **连续失败阈值**：默认 `3`；说明“当前 Key 失败后继续重试；真实失败连续达到此次数后打开熔断并尝试下一把 Key，计数跨请求保留”。
...
- `maxRetryCount` 必须在 `0..3`，默认 `3`；它表示首把 Key 之外的额外 Key 数量。raw outbound attempt 另由 `min((1 + maxRetryCount) * consecutiveFailureThreshold, SYSTEM_RAW_ATTEMPT_HARD_CAP)` 约束，不能把内部上限误认为用户语义；
...
| 旧字段 | v3 处理 |
| --- | --- |
| `maxTotalAttempts=4` | `maxRetryCount=3`，迁移为“首把 Key 外最多再尝试 3 把 Key”；migration audit 记录语义从 raw attempt 数变为不同 Key 数 |
...
- `maxRetryCount=0/1/3` 分别严格对应最多 1/2/4 把不同 Key；它不限制同一 Key 在熔断前的 raw attempt 数。
- `consecutiveFailureThreshold=3` 时，Closed Key 的普通可重试失败序列必须是 `A, A, A`，第三次失败使 A Open；若 `maxRetryCount` 仍有名额，下一次才是 B。默认两把 Key 都持续失败时序列为 `A, A, A, B, B, B`。
- 429、上游 5xx、凭据/普通请求拒绝、已跨 outbound boundary 的连接失败、首字节超时在 replay-safe 且未 commit 时先重试当前 Key；达到阈值后才重新排序并尝试下一把 Key。`429` 不触发站点/容量域级别的等待或排除。
- `429` 的处理与其他普通上游错误一致：每次只标记当前 `station_key_id` 的 raw failure；响应中的 `Retry-After` 不改变 Key 数量预算、候选顺序或冷却算法。
- 仅容量准入拒绝时不视为 outbound attempt；继续向后准入不消耗 `maxRetryCount`，但仍受 deadline 和系统候选硬上限限制。
- 严格余额不足、严格确认当前 Key 不支持请求模型时直接结束，不重试也不换 Key；其他普通上游错误统一走同 Key 阈值路径。已提交响应仍由请求安全边界停止。
- 出站边界前的本地连接/DNS/序列化/适配器错误不自动换 Key、不消耗 retry，也不写 Key 失败或 circuit；只有已跨边界的传输失败才走 Key 级重试。
- 同一请求不会无限重复同一 Key：Closed Key 受连续失败阈值约束，Half-Open Key 只允许一次真实请求，整个请求还受 deadline 和内部 raw attempt hard cap 约束。本地容量准入拒绝不进入同目标等待，直接按容量规则继续向后找候选。
...
- 可靠性和响应速度由整把密钥的实际路由和监控样本先分别计算，再按来源权重混合；模型不同不会阻止监控样本进入密钥级监控来源；历史最小样本默认 15，最近最小样本默认 5，窗口样本不足时使用可调乐观值。
...
3. **来源权重与密钥级聚合**：评分作用域固定为整把密钥。监控请求按 `station_key_id + lifecycle_revision` 进入独立监控来源池，不因模型、endpoint variant 或请求形态不同而排除；可靠性和响应速度来源先独立计算，再按设置中的实际流量/监控权重混合。
