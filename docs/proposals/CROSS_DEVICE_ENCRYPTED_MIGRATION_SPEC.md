...
- 路由、采集频率、既有 channel monitor 模板配置和用户设置；运行态字段按本表规则清理
...
| `station_keys` | `IncludeWithTransform` | 保留 Key 元数据和 secret 引用；明文兼容 API Key 列必须为空；不涉及 channel monitor 运行态 |
...
| `channel_monitor_request_templates` | `IncludeWithTransform` | 仅作为既有配置数据保留；请求体通过禁止敏感字段校验后保留 |
| `channel_monitors` | `IncludeWithTransform` | 仅保留既有配置；清空 last/next run 和临时错误字段；不得迁移运行态 |
...
- 可携带：路由策略、倍率限制、余额阈值、采集/价格刷新周期、超时、并发、既有 channel monitor 配置项（不含运行态）和用户 UI 偏好。
...
`restore_into_empty` 不能只检查 `stations = 0`。必须由 `MigrationDataCatalog` 为每张用户拥有的表定义 occupancy 查询；只有 Station、Key、非设备 secret、自定义路由/价格、既有 channel monitor 配置、通用登录资料和 draft 等用户数据全部为空，且数据库只含 binary 创建的默认设置与 built-in 行时才视为空。发现未知表或未知 setting key 时不得判空。
...
2. 写入不依赖 secret 的根记录：Station（secret 引用暂置空）、model alias、model base price、既有 channel monitor 模板配置和已允许的普通设置；不得写入或恢复运行态。
