...
D-01 through D-09 are marked closed in `docs/superpowers/audits/2026-07-31-schema15-upgrade-debt-manifest.json`. The RustSec advisory gate passes when PowerShell is run through the local proxy at `http://127.0.0.1:7890`. The release version contract also passes when `RELAY_POOL_RELEASE_TAG=v0.3.3` is set. Because this cleanup is not being published as a release in this turn, Tauri release signing credentials are recorded as a publish-time requirement rather than a cleanup blocker.
