...
Status: Task 28 debug legacy/request-coupled runtime deletion applied under the development reset/reimport recovery decision; Task 26/27 development self-checks reran successfully on 2026-08-01 after the deletion work, with real provider/client observations still authorization-gated and recorded as not run
Date: 2026-07-31
...
|---|---|---|---|---|
| Legacy weighted score path | `src-tauri/src/application/routing_engine/routing_policy.rs`, `scheduler/scoring.rs`, `scheduler/selection.rs` deleted; `routing_engine/mod.rs` no longer exports `router`, `routing_policy`, or `scheduler` | Multi-factor score and TopK weighted order conflict with sealed hierarchical kernel | Task 12/22 | Deleted from production module tree; only historical ledger text and architecture red fixtures may reference old names |
| `cheap_first_score` input/output addition | `routing_policy.rs::estimated_cost` deleted with the legacy weighted score path | Adds input and output token unit prices without a request-time scalar basis | Task 7/12 | Deleted; cost ordering must use request-scoped projection/planner facts |
| Simulated capacity | legacy scheduler module deleted; `acquired_simulated` production symbol removed | Candidate can be selected without owning executable capacity | Task 15/22/24 | Deleted; default-v2 production uses `CompositeCapacityRegistry` and RouteAdmissionController |
| Slot unavailable in ordered IDs | legacy scheduler module deleted; `slot_unavailable` production symbol removed | Unavailable candidate remains executable intent | Task 15/16/22/24 | Deleted; RoutePlan unavailable intent never becomes default-v2 execution |
| Test-only scheduler feedback facade | `SchedulerRuntimeState`, legacy affinity store, and legacy metrics registry deleted with `scheduler/` | Production cannot use same runtime feedback and affinity contract as tests | Task 14/19/22 | Deleted; outcome consumers and runtime metrics must use the new controller/runtime modules |
...
- Task 24 pre-deletion approval is a development self-check, not a product-release workflow. Required evidence is production composition, stream/drop lifecycle faults, redaction contracts, and a single-pass deterministic Task 21 loopback soak, written only under ignored
  `output/routing-operational/self-check/task24-predeletion/`.
  ...
  Historical note: an earlier clean long run on 2026-07-31 started at `2026-07-31T15:39:56.9784240+08:00`, finished at `2026-07-31T16:40:05.1488150+08:00`, ran at commit `42367bf0398e00afd1ded4c149212ff00a02a970`, had `worktreeCleanAtStart = true`, and wrote `deletionApproved = true` in `task24-predeletion-gate-latest.json`; this remains optional confidence evidence for debugging lifecycle leaks.
...
- Routing workspace/simulation deletion follow-up on 2026-07-31 removed the remaining legacy weighted scheduler/router production module tree:
  `src-tauri/src/application/routing_engine/router.rs`,
  `src-tauri/src/application/routing_engine/routing_policy.rs`,
  and `src-tauri/src/application/routing_engine/scheduler/`.
  The only behavior kept from the old module is model alias lookup, moved into narrow pure helper `src-tauri/src/application/routing_engine/model_alias.rs`.
  `RoutingService::simulate_route`, `RoutingService::load_local_routing_workspace`, and `routing_snapshot.rs` preview rows now consume `RouteCandidateProjection`/hierarchical planner facts instead of old scheduler decisions.
  `scripts/routing-single-owner.test.mjs` now rejects reintroducing the deleted routing scheduler/router module exports and requires application routing previews to use projection + planner.
- Task 28 deletion is allowed by the 2026-07-31 development recovery decision: this is not a stable product, so the supported recovery remains reset local data, reimport config, or reconfigure with the current dev binary. Real/manual observations still improve confidence but do not justify keeping a second runtime/finalizer alive.
...
- Current automated evidence:
  - Task 27 local deterministic self-check runner `scripts/run-routing-operational-local-self-check.ps1` exists and writes ignored evidence under `output/routing-operational/self-check/local-self-check/`.
  ...
  - Local OpenAI-compatible client smoke harness `scripts/verify-local-routing-lifecycle.ps1` now requires `-AuthorizeLocalClientSmoke`, writes ignored redacted evidence under `output/routing-operational/self-check/local-client-smoke/`, and verifies request lifecycle rows through `scripts/verify-request-lifecycle-db.ps1` when DB verification is enabled.
