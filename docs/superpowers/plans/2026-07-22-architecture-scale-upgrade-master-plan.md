...
**当前 checkpoint：** settings/stations、key-pool/stationKeys、changeEvents/collectorRuns、external URL、proxy/localRouting、dataRecovery、economics、groupFacts 与 pricing workspace 子切片已建立 `DesktopBackend` domain client；legacy `src/lib/api/settings.ts`、`src/lib/api/stations.ts`、`src/lib/api/stationKeys.ts`、`src/lib/api/changeEvents.ts`、`src/lib/api/collectorRuns.ts`、`src/lib/api/external.ts`、`src/lib/api/proxy.ts`、`src/lib/api/localRouting.ts`、`src/lib/api/dataRecovery.ts`、`src/lib/api/economics.ts`、`src/lib/api/groupFacts.ts` 与 `src/lib/queries/pricingQueries.ts` 已删除 `isTauriInvokeUnavailable`、browser-preview memory fallback、内存 production state 和直接 generated/transport/streaming adapter import；DemoBackend 对上述真实桌面能力返回 typed unsupported，station-key connectivity 在 demo 下不得 fake success。剩余 `channelMonitors`、`collector`、`routing` 与 `channelQueries` feature 仍待迁移，Stage 2 Gate 不得视为通过。
