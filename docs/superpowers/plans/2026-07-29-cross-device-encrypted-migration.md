...
当前基线必须在 Task 0 重新确认：database generation `2`，writable schema、应用表数量和最新 migration 均以当时 registry 为准。状态监控合并后 `0009_provider_drafts.sql` 与 `0010_status_monitoring_v2.sql` 已被占用；安全基线 migration 应选下一个空闲编号（当前应为 `0011_encrypted_secret_baseline.sql`），并同步更新本计划执行记录、fixture 和 compatibility 常量，禁止覆盖已有 migration。
...
- Create after preflight confirms free: `src-tauri/src/persistence/migrations/0011_encrypted_secret_baseline.sql`
