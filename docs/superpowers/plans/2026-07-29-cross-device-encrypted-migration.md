...
当前基线必须在 Task 0 重新确认：database generation `2`，writable schema `9`，当前 schema 有 29 张应用表，`0009_provider_drafts.sql` 已存在但工作区未提交。安全基线 migration 只有在编号仍空闲时使用 `0010_encrypted_secret_baseline.sql`；若已被占用，停止该 task，选下一个编号，并同步更新本计划执行记录、fixture 和 compatibility 常量，禁止覆盖已有 migration。
...
- Create after preflight confirms free: `src-tauri/src/persistence/migrations/0010_encrypted_secret_baseline.sql`
...
测试 schema 9 有/无 secrets、有 legacy Local Key、`stations.api_key`/`station_keys.api_key`/`station_credentials.login_password` legacy 明文、明文与现有 secret 冲突、错误密文、错误 key、backup 失败、每个 journal 原子边界崩溃、target validation 失败、active replacement 未知、旧 binary compatibility。断言 active v9 从不被 sqlx migrator 原地推进到 v10。

- [ ] **Step 2: 添加结构 migration，但禁止直接激活半成品**

`0010` 只提供新库构建所需结构：可回填的 `key_id/encryption_version` 中间形态与 `app_secret_bindings`。schema compatibility 仍保持 pre-baseline，直到 application conversion 重建 `secrets` 为最终 `NOT NULL`、验证全部密文、加密 Local Key、迁移并清空三个 legacy credential 列及 `settings.local_key`、完成重建/VACUUM 后才提交新 profile。明文与已有 secret 冲突时 fail closed；fresh database 也走同一 finalizer。
...
setup 在 persistence runtime 之前检测 schema/profile：v9 进入 baseline converter；v10 完整 profile 才 writable；半转换或 journal 矛盾进入 recovery UI。旧 binary 看到 v10 必须 compatibility fail closed。
