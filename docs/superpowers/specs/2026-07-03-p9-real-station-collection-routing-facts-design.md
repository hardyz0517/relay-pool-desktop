...
- `CollectedBalanceFact`
  包含 station_id、station_key_id、scope、value、used、total、currency、credit unit、status、source、confidence、collected_at。

- `CollectedGroupFact`
  包含 station_id、group_id、group_name、visibility、source、confidence、raw_json_redacted。

- `CollectedRateFact`
  包含 station_id、station_key_id、group_id、group_name、default_rate_multiplier、user_rate_multiplier、effective_rate_multiplier、source、confidence、checked_at。
...
当前项目里用户可能把 `base_url` 填成站点根地址，也可能填成 OpenAI-compatible 转发地址，例如以 `/v1` 结尾。P9 必须在 collector 层引入统一 URL helper：

- `upstream_api_base_url`：用于请求转发和 `/v1/models`。
- `management_base_url`：用于 Sub2API / NewAPI 的后台接口，例如 `/api/v1/groups/available`。
- 当用户输入 `https://example.com/v1` 时，management base 应归一化为 `https://example.com`，避免拼出 `/v1/api/...` 或 `/v1/v1/usage`。
- 当站点确实把管理接口挂在自定义路径下时，后续通过 adapter config 支持，不在 UI 层写特判。

余额采集和 key group resolve 应优先使用 `station_keys` 中的真实 routeable API key。`stations.api_key` 只作为旧数据和默认 key 的兼容来源；如果已存在默认 Station Key，就不要再把 Station 当作路由 key 使用。
...
```text
GET {upstream_api_base_url}/usage
Authorization: Bearer {apiKey}
```

如果 helper 得到的是站点根地址，则等价调用 `{management_base_url}/v1/usage`。实现必须通过 URL helper 生成最终 URL，不允许手写字符串拼接。
...
NewAPI 同样使用 URL helper。`/api/user/self` 和 `/api/user/self/groups` 应基于 `management_base_url`，不能基于可能带 `/v1` 的转发 URL。
...
- `allow_depleted_fallback`，默认 false。
