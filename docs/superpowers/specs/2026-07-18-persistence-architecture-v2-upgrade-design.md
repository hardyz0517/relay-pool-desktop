...
- `v0.1.0/v0.2.0` 不理解新 lease，自动升级只支持 updater 先完整退出旧进程、再启动 V2 的受控路径。任意复制的旧 executable 与 V2 side-by-side 运行不属于受支持合同；release 文档和 upgrade harness 必须明确阻断该路径。
...
- 使用签名包验证双实例、进程崩溃 lock 自动释放、`v0.1.0/v0.2.0` 受控 updater 退出、旧 binary 对新 schema/tombstone 在写入前 hard fail。
...
21. 同一 installation 的第二个 V2 进程在任何 config/journal/SQLite/Proxy/Collector 副作用前失败；owner 正常退出或崩溃后 OS 自动释放 lease，无 stale-lock 删除逻辑。`v0.1.0/v0.2.0` 通过 signed updater 完整退出后升级，source 在 tombstone 前再次证明未变化。
