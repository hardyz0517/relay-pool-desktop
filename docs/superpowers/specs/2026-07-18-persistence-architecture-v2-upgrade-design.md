...
4. worker 调用 `RequestFinalizationService`，在一个 transaction 内完成 endpoint revision 校验、request-log 幂等插入和 health feedback。只有 request-log insert 实际插入一行时才允许写 health feedback。
...
- 启动发现 pending migration 时，先完成当前 V2 数据库 health check 和 8.5 的 verified backup；backup 失败、空间不足或校验失败时不执行 migration、不启动业务 runtime。
...
Journal 存在但 version/checksum/shape 无法验证时，或发现没有匹配 journal 的 inactive V2 final file / tombstone 时，必须保留所有 artifact 并 fail closed，提供脱敏 diagnostic；禁止按时间、文件大小、row count 或文件名猜测并自动覆盖、激活或删除。恢复首先验证 phase、config、source、backup、candidate、tombstone 的组合，只允许下列显式状态；未列出的组合均 fail closed。
...
1. 使用现有 fail-closed Data Store 边界解析权威 data dir，并确认不存在未完成 relocation。
2. 若已存在 upgrade journal，先按 phase 执行确定性 recovery，不得开始新的 attempt；若没有 journal 但存在 inactive V2 final file 或 tombstone，则进入 orphan recovery，不得创建新 attempt。
3. Read-only 打开源库并识别 released schema profile。
4. 创建 `Prepared` journal。
5. 执行 source `quick_check`、integrity check 和可用的 foreign-key inspection。
6. 使用 SQLite 一致性备份创建受保护 backup；reopen 验证并计算 SHA-256 后持久化 `BackupVerified`。
7. 仅在 journal 证明文件属于本 attempt、位于选定 data dir 且未激活后，删除 inactive V2 temporary file。
8. 创建 generation-2 temporary DB 并应用 embedded migrations。
9. 按依赖顺序导入 canonical configuration、secret ciphertext、reference 和 history。
10. 重新计算 projection 和仍需保留的兼容事实，不盲目复制 cache。
11. 验证完整 V2 数据库并持久化 `V2Validated`。
12. 关闭 source 和 temporary connection。
13. flush validated temporary file，原子 rename 为 generation-2 fixed filename，并 sync parent directory。
14. 再次验证 backup SHA-256 后，按 tombstone 合同 deactive generation 1，并持久化 `LegacyDeactivated`。
15. 原子提交 `database_generation = 2`，sync config parent 后持久化 `GenerationCommitted`。
16. 通过正常 V2 runtime reopen，并再次执行 startup health check。
17. 持久化 `V2Reopened` 并安全删除 journal。
18. 成功后才注册 Application Service、Proxy、Collector、Monitor 和正常 Command。
...
只有 `Busy` 和明确分类的 transient I/O 可以在总 attempt/time budget 内重试。`CommitOutcomeUnknown` 必须先按幂等键或 state precondition 读取并对账；Integrity、Migration、Secret 和 invariant failure 禁止伪装成 empty state 或 fallback data。
...
在以下边界逐一注入失败：source open、integrity check、backup、temporary create、每个 import phase、checksum、secret validation、close、file activation、每个 UpgradeJournal phase 持久化、tombstone create/flush、config commit、V2 reopen、journal cleanup、service registration。
