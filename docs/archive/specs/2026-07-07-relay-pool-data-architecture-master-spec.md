...
配套字段登记簿：`docs/audits/relay-pool-field-ownership-ledger.md`。新增字段、漂移字段、准备降级字段都必须先登记到该文件。
...
如果 query service 必须读取兼容字段，只能作为搬运字段返回，并且对应字段必须登记在 `docs/audits/relay-pool-field-ownership-ledger.md`。
...
- 新字段默认进入 `docs/audits/relay-pool-field-ownership-ledger.md` 的 `unknown pending audit`，不得立即删除、合并或重命名。
