...
- 字段归属清单路径：`docs/audits/relay-pool-field-ownership-ledger.md`
- 当前状态：已建立核心字段初始登记。
...
- 新增总 SPEC、总计划、字段归属清单和 Stage 0 执行计划，防止上下文压缩后方向漂移。
- 新增字段所有权扫描，显式冻结兼容字段直接消费白名单。
- 收紧价格分组 identity 测试，保护 `group_key_hash` 和 `group_id_hash` 不被误合并。
- 修复价格页绑定行 rate 选择：同一 current group 下仍消费 shadow rate 防重复，但展示优先选择 exact `groupBindingId` 的 rate。
- 收紧 Dashboard 余额、Edit Key 分组绑定、shared capabilities group 字段契约测试。
