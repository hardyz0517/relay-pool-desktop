...
> **执行者必读：** 本计划是 `docs/archive/specs/2026-07-22-architecture-scale-upgrade-design.md` 的唯一总实施入口。开始任何 Task 前必须先读设计 spec、本文、当前 Task 引用的 ADR/清单，并核对工作树。不得跳过前置门禁后直接拆文件。
...
**批准的设计：** `docs/archive/specs/2026-07-22-architecture-scale-upgrade-design.md`
...
- Create: `docs/archive/audits/2026-07-22-architecture-scale-upgrade-baseline.md`
- Create: `docs/audits/architecture-scale-upgrade-inventory.json`
- Create: `docs/audits/architecture-scale-boundary-manifest.json`
- Create: `docs/audits/architecture-scale-threat-model.md`
- Create: `docs/audits/architecture-scale-tauri-security-manifest.json`
- Create: `docs/audits/architecture-scale-dependency-lifecycle.json`
- Read only: `docs/archive/specs/2026-07-22-architecture-scale-upgrade-design.md`
...
Stage 0 必须实际生成固定 seed、稳定排序、脱敏的 10/100/500 stations/keys fixture，并取得 frontend invoke count、projected response JSON bytes、TanStack Query lifecycle 和 data-ready React commit 的原始样本。当前没有 runtime measurement owner 的 `backend_read_port_round_trips`、`backend_sql_statement_count_runtime`、`backend_query_duration_ms`、`real_tauri_ipc_payload_bytes`、`real_tauri_command_duration_ms` 和 `webview2_page_commit_ms` 必须写成 `value: null, qualification: blocked`，分别登记 owner Task 11 和 release gate Task 26；禁止填 0 或用源码静态计数冒充实测。`sqlite_query_plan` 由 Stations aggregate read focused Rust test 提供，不作为 frontend baseline blocked metric。Stage 0 可在这些 blocker 有唯一 owner/关闭 Task 时建立基线，但 Stage 3/7 对应 Gate 在 blocker 关闭前不得通过。
...
- Create: `docs/adrs/architecture-scale/0001-ipc-contract-and-error.md`
- Create: `docs/adrs/architecture-scale/0002-application-composition.md`
- Create: `docs/adrs/architecture-scale/0003-backend-mode.md`
- Create: `docs/adrs/architecture-scale/0004-query-and-page-visibility.md`
- Create: `docs/adrs/architecture-scale/0005-work-lifecycle.md`
- Create: `docs/adrs/architecture-scale/0006-provider-registry-and-outbound.md`
- Create: `docs/adrs/architecture-scale/0007-ci-and-artifacts.md`
- Create: `docs/audits/architecture-scale-capacity-budgets.json`
...
- Create: `deny.toml`
- Create: `docs/audits/dependency-advisory-exceptions.json`
...
- [x] 建立 10/100/500 deterministic frontend scale baseline：fixture 双次生成 hash 一致，active backend/mock transport 记录 command/count/projected response JSON bytes，独立 QueryClient 记录 lifecycle，React Profiler/浏览器时间线记录 data-ready commit；warm-up/sample/provenance 完整。未具备 backend/Tauri/WebView runtime owner 的指标保持 typed blocked，并由 Task 11/26 关闭。
...
- [x] 删除 Stations per-row `useQueries` 和 Key Pool 本地 server-state 副本。
...
**Stage 3 Gate：** Query Cache 是 server state 唯一 owner；aggregate refresh 有界/O(1)；页面可见性唯一；所有长期 draft/operation owner 明确。当前 source/jsdom topology 证据已通过：`page-activation-refresh`、`hidden-page-query-boundary`、`page-activity-query-contract`、`query-client-contract`、`query-services-boundary`、`page-switch-data-path-performance`、`remote-key-keypool-refresh`、`log-change-shared-query`、`stations-bounded-snapshot-query`、`station-asset-loading-boundary`、`stations-page-activity-query` 和 `architecture:scale-baseline` 均为绿；10/100/500 frontend command 数固定为 4，hidden query starts 为 0。未包含 packaged/native runtime SQL/read-port、真实 Tauri IPC 和 WebView2 timeline；这些仍是 Task 26/28 release qualification 的硬门禁。
...
**当前 checkpoint：** `S4-T16C-manual-station-collection-blocking-executor` 将手动 station collection prepare 和 `test_station_login_input` 的阻塞探测接入共享 `BlockingExecutor`。`StationCollectionCommandFacade` 通过 composition 显式接收同一个 managed blocking executor；manual collection / login prepare 由 bounded queue、queue timeout、execution timeout 和 orphan diagnostics 治理，命令层将 blocking 失败映射为稳定 public work errors，并删除对应 direct `spawn_blocking` allowlist 项。该片尚未把 manual collection 提升为 OperationRegistry foreground operation。

**当前 checkpoint：** `S4-T16D-station-collector-scheduled-blocking-executor` 将 scheduled station collector 的 `collect_task` prepare 阻塞段接入共享 `BlockingExecutor`。`v2_runner_port` 通过启动 composition 接收 managed blocking executor；每次 Balance/Groups prepare job 记录 `station-collector-runner:<run_id>` operation 标识，并在 `TaskRunContext` cancellation 时取消 queued/running blocking handle；旧 `services::station_collectors::<impl>::collect_task` direct `spawn_blocking` allowlist 项已删除。该片不迁移 channel monitor/capture/shutdown，也不改变 collector apply/persistence side effect。
...
**当前 checkpoint：** `S4-T17C2-exit-coordinator-cutover` 新增 `background_tasks::exit::ExitCoordinator` 并迁移 tray Quit、main window true-close 和 Tauri `RunEvent::ExitRequested`。首次 exit request 在可 prevent 阶段启动 bounded async drain；drain 停止 monitor/collector runner、drain proxy、关闭 supervisor/blocking executor 并释放 data-store owner 后，coordinator 标记 final exit 并只触发一次 `app.exit`。`RunEvent::Exit` 不再执行主要 async shutdown 或 `block_on`。该片不改变 updater install/restart API，也不接管 proxy request lifecycle。
...
**Checkpoint addendum:** `S4-T18G-proxy-request-correlation` gives each v2 proxy request a deterministic bounded correlation id derived from its existing request id, enters that scope around executor routing/execution/failure finalization, and carries the same scope through response body stream polling/finalization. Focused tests prove the derived id is stable/redacted, does not leak after request work, is visible inside routing repository execution, and remains present while the lifecycle-finalizing body polls its inner stream. This does not close Stage 4; remaining spawn/HTTP allowlist disposition is still open.

**Checkpoint addendum:** `S4-T18H-owned-spawn-http-allowlists` converts the parser-enforced spawn and HTTP construction allowlists from bare string identities into owned entries with exact identity, owner, reason, introduced shard, delete shard and expiry stage. The boundary test now rejects unowned allowlist strings, duplicate identities, expired entries and stale identities while still requiring every production-cfg spawn/client construction site to be listed. Stage 4 infrastructure spawns are classified as managed runtime owners; legacy remote-key, endpoint-ping, updater-manifest and connectivity transport sites are explicitly assigned to Stage 5 Task 20-22 deletion. This does not start Stage 5; a final Stage 4 gate audit is still required before moving on.

**Checkpoint addendum:** `S4-T18I-bounded-metrics-ttl-contract` freezes the Stage 4 metric contract for local diagnostics. `MetricKind::stage4_required()` now enumerates command latency/error, workspace latency/payload/IPC count, task status/backoff/shutdown timeout, operation terminal/cancel latency, blocking saturation/orphan, collector failure, hidden query start and binding drift. `LocalMetricBuffer` now supports TTL-based garbage collection in addition to fixed capacity, and metric label validation rejects secret-bearing, URL/query-like and unbounded free-form labels. The accepted capacity budget records `diagnostics.in_memory_event_ttl_ms=600000`, and focused observability tests prove coverage, GC and label rejection.

**Checkpoint addendum:** `S4-T18J-runtime-diagnostics-exposure-boundary` freezes the public exposure boundary for local runtime diagnostics. `RuntimeStatusDto` serialization now has an exact key allowlist for the root object and task projection, rejecting metrics/blocking/outbound/operation/raw diagnostics fields, and the compiled IPC registry proves `get_runtime_status` remains a read-only `EmptyInputDto -> RuntimeStatusDto` command while no public command exposes `RuntimeDiagnostics`, `RuntimeDiagnosticsSnapshot` or metric snapshots. This shard does not add cloud telemetry or developer-mode diagnostics UI; it only prevents accidental public exposure before any future developer-only surface is designed.

**Checkpoint addendum:** `S4-T18K-blocking-network-operation-contract` adds gate evidence for two parts of the final Task 18 work/outbound item. The parser-backed architecture boundary now records production `BlockingExecutor::submit` job kinds and rejects network-shaped work names from that executor, keeping legacy HTTP/probe/provider paths on explicit spawn/HTTP allowlists until Stage 5 deletion. Operation DTO tests now assert exact public snapshot/progress/terminal/cancel outcome shapes so join handles, cancellation tokens, commit barriers, instants, paths or secret fields cannot drift into operation status/cancel IPC. This is partial evidence only: the realtime operation event envelope id/version/terminal contract still needs its own closure before Stage 4 Gate can close.

**Checkpoint addendum:** `S4-T18L-structured-event-redaction-contract` freezes the structured span/event boundary. `observability::events` now accepts only stable public code/kind/duration/result plus optional hashed redacted resource id, rejects URL/query/path/token/cookie/Authorization-shaped codes, and proves `Debug` output cannot contain raw resource material. `correlation` validates command/work span fields through the same stable-code contract before creating `tracing::info_span!`, and the parser-backed architecture boundary permits production tracing macros only in `observability::correlation`. Metrics label redaction remains covered by `S4-T18I`; this shard closes the remaining structured span/event redaction item without adding cloud telemetry or a diagnostics UI.

**Checkpoint addendum:** `S4-T18M-operation-event-envelope-contract` freezes the foreground operation event envelope without introducing a new streaming transport. `OperationEventDto` carries `schemaVersion`, stable `eventId`, `operationId`, monotonic `sequence`, and a tagged `progress | terminal` event body; terminal is explicit and independent from progress/channel close. The generated TypeScript contract and binding hash now include the event DTO, and existing registry tests still prove `get_operation_status`, `cancel_operation`, and `start_station_key_connectivity_operation` have their read/idempotent/non-idempotent semantics and result-unknown owner. Together with `S4-T18K`, this closes the Task 18 work/outbound checklist item while leaving Stage 5 blocked until the final Stage 4 Gate audit is run against the current revision.
...
- [x] 建立 command、task run、operation、collector run、monitor run、outbound request 和 proxy request 的 correlation propagation test；跨层不生成无关联新 id。
- [x] 结构化 span/event 只使用稳定 code、kind、duration、result、redacted resource id；完整 key/cookie/token/Authorization、prompt/response body、provider payload、URL query 和数据库路径在 Debug/Display/event/metric 中均不可达。
- [x] 本地 bounded metrics 至少覆盖 command latency/error、workspace latency/payload/IPC count、task status/backoff/shutdown timeout、operation terminal/cancel latency、blocking saturation/orphan、collector failure class、hidden query start 和 binding drift。
- [x] metrics label 必须是封闭低基数 enum/normalized id；禁止 station id、operation id、URL、model 自由文本作为无界 label。诊断 buffer 具有 Task 1 冻结的容量/TTL，并验证 GC。
...
- [x] runtime status/read model 只暴露用户可行动状态；完整本地诊断仅开发者模式可读，仍经过同一 redaction contract。本升级不引入云遥测。
- [x] production 永久 runner 中 `thread::spawn + block_on` 为零；fire-and-forget spawn 只有带 owner/理由/删除期限的最小 allowlist。
- [x] production network I/O 不允许进入 BlockingExecutor；operation event 必须带 id/version/terminal contract，cancel command 必须有 registry owner。
...
- Create: `docs/audits/provider-capability-matrix.json`
...
- Create: `docs/archive/audits/2026-07-22-architecture-scale-deterministic-qualification.md`
...
- Create: `docs/archive/audits/2026-07-22-architecture-scale-soak-live-qualification.md`
...
- Create: `docs/archive/audits/2026-07-22-architecture-scale-upgrade-final-qualification.md`
