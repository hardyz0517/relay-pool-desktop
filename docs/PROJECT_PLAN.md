...
`Station` 是站点账号资产，表示一个中转站网站 + 一个 OpenAI-compatible API namespace + 一个用户登录账号。

它负责：

- name
- website_url：浏览器登录、网页登录授权、站点管理和采集入口的前端网址
- api_base_url：Station Key、PING、渠道监控和本地路由使用的完整 OpenAI-compatible API namespace
- endpoint_revision：端点配置版本，用于拒绝旧后台任务写回当前健康、凭据或采集事实
- station_type
- 登录账号 / 密码状态
- 登录状态
- 余额来源
...
`Station` 不是最终请求路由对象。
管理端点选择跟随端点角色，而不是采集任务名称：登录、远端 Key 管理和站点管理调用使用 `website_url`；模型探测、Key 连通性、渠道监控和代理转发使用 `api_base_url`。
...
本地网关默认且唯一运行 v2 runtime。旧 debug legacy runtime、`RELAY_POOL_PROXY_RUNTIME=legacy` 入口和 request-coupled finalization 分支已按开发期 reset/reimport/重新配置恢复策略删除；它们不是 UI 设置、不是长期产品能力，也不作为结构性 blocker 的自动 fallback。当前项目仍处于非稳定成型阶段，不维护 signed installer、安装升级矩阵或旧二进制 rollback gate；若未来进入稳定产品阶段，再由独立发布 ADR 重新定义真实发布、升级和回滚要求。
...
Files、Batches、Audio、Images、Realtime、Assistants 等端点不在本地网关当前版本的路由范围内；客户端请求这些端点时不应被默默转发或伪装支持。
