...
> Relay Pool Desktop 是一个本地 AI 中转站与 Key 池调度器：对外提供固定 OpenAI-compatible 入口，对内管理多个 Sub2API / NewAPI / OpenAI-compatible 中转站账号及其下的 API Key，自动采集余额和倍率，并根据 Key 池优先级、模型能力、协议能力和健康状态进行本地路由。

P5 已经落地本地 OpenAI-compatible 网关主干：外部工具可以连到 `127.0.0.1`，请求会按 Key 池优先级和协议格式做 fallback 路由，日志也会真实落库；`/v1/models` 会聚合并去重 enabled Key 的模型列表，`/v1/chat/completions` 与 `/v1/responses` 已接入基础 SSE 流式透传。

P6 已经把路由从“按优先级 fallback”升级为模型感知、协议感知、健康感知的 Station Key 路由层：支持模型 alias、Key 能力范围、allowlist / blocklist、冷却、稳定优先、备用模式、路由模拟器和请求日志解释。价格归一化、余额避让、最低价路由和复杂策略 DSL 后移到后续阶段。
...
找到支持该模型的 Station Key

&#x20; ↓

过滤禁用、余额不足、熔断中的 Station Key
...
\### Phase 5：本地 OpenAI-compatible 网关
...
\* `/v1/chat/completions` 和 `/v1/responses` 转发；

\* SSE 流式透传；

\* 按 Key 池 priority 做基础 fallback；
...
\* CCSwitch / Codex / curl 能连接本地入口；

\* 上游失败后能按候选排序 fallback；

\* 请求日志不保存 prompt、response 或完整 key。
...
\* Station Key 模型 allowlist / blocklist / preferred models；

\* 基于 request logs 的 health / cooldown；
...
\* 请求能根据 endpoint、model、stream、tools、vision、reasoning 过滤 Station Key；

\* 冷却中的 Key 不再被反复撞；

\* 路由规则页能解释为什么选中或拒绝某把 Key；

\* 请求日志能展示 route policy、selected reason 和 rejected candidates。
