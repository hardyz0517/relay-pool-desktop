...
- **让采集事实直接参与运行时决策**：将余额、分组、倍率、模型能力、健康和价格整理为本地事实，再用于候选筛选、排序、冷却和 fallback，而不是只保存一组静态 Base URL 与 Key。
...
当前最新版本是 **v0.2.2**。Windows 用户可以从 [GitHub Releases](https://github.com/hardyz0517/relay-pool-desktop/releases/latest) 下载对应的 NSIS 安装包；已安装版本也可以在应用内设置页手动检查更新，更新元数据来自公开的 `latest.json`。
