...
- P5 已具备本地 OpenAI-compatible 网关骨架，按 Key 池优先级 fallback，聚合模型列表，并支持非流式与 SSE 流式透传；后续继续扩展价格和健康策略。
...
- 具体价格归一化、复杂策略和 WebView 登录捕获会在后续阶段按模块逐步补强；当前已具备本地网关核心、request log 落库、渠道状态轻量统计和基础流式透传。
