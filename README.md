...
当前状态：early development。Phase 2 已完成本地 SQLite 数据层和 stations/settings 持久化；Phase 2.5 已完成柔和桌面控制台 UI reset；Phase 3 正在引入 station account、多 API key 数据模型和 collector prototype；P3.1 已将采集页调整为“信息采集”控制台，并开始抽象通用 Collector Adapter。项目尚未实现真实代理、请求转发、真实路由或完整 WebView 登录捕获。
...
- Phase 2 已接入本地 SQLite，持久化中转池站点和部分本地设置。
