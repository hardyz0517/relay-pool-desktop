...
use models::pricing::{PricingStatus, UpsertBalanceSnapshotInput, UpsertModelBasePriceInput};
...
fn cost_first_uses_multiplier_proxy_when_exact_prices_are_missing() {
    ...
    resolution.group_binding_id = Some("binding-1".to_string());
    resolution.group_rate_multiplier = Some(0.42);
    resolution.group_confidence = Some(0.8);
    resolution.group_collected_at = Some("300".to_string());
