...
async fn test_pool() -> SqlitePool {
    ...
    sqlx::query(
        ...
        INSERT INTO persistence_schema_compatibility (
            singleton_key, database_generation, schema_version,
            min_reader_app_version, min_writer_app_version, updated_by_migration, updated_at
        ) VALUES (1, 2, 18, '0.3.3', '0.3.3', 18, '1');
        "#,
    )
    .execute(&pool)
    .await
    ...
    sqlx::query(include_str!(
        "../src/persistence/migrations/0019_routing_decisions.sql"
    ))
    .execute(&pool)
    .await
    .expect("migration");
