...
use application::credentials::{SecretBytes, SecretRef};
...
use target_resolver::{
    ExecutionCredentialResolver, ExecutionTargetError, ExecutionTargetRef, ExecutionTargetResolver,
    ...
    fn resolve_station_key_secret_ref(
        ...
        Box::pin(async move {
            if secret_ref.owner_id != station_key_id {
                return Err(ExecutionTargetError::SecretUnavailable { station_key_id });
            }
