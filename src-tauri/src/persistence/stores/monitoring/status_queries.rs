...
use crate::{
    models::monitoring::{
        ChannelMonitorAttemptCursor, ChannelMonitorAttemptHistoryInput,
        ChannelMonitorAttemptRecord, ChannelMonitorExecutionCursor, ChannelMonitorExecutionIdInput,
        ChannelMonitorExecutionListInput, ChannelMonitorExecutionSummaryV2,
        ChannelMonitorTargetResultRecord, ChannelStatusBucketCounts, ChannelStatusEndpointPing,
        ChannelStatusOutcome, ChannelStatusRecentPoint, ChannelStatusRunningExecution,
...
#[derive(Debug, Clone)]
pub(crate) struct StatusBaseRow {
    pub(crate) monitor_id: String,
...
#[derive(Debug, Clone)]
pub(crate) struct WorkspaceRollupCell {
...
#[derive(Debug, Clone)]
pub(crate) struct WorkspaceDirtyRange {
...
impl MonitoringStatusQueryRepository {
    pub(crate) async fn workspace_base_rows(
        ...
        max_rows: i64,
    ) -> Result<Vec<StatusBaseRow>, PersistenceError> {
    ...
    pub(crate) async fn workspace_rollups(
        ...
        range_start_ms: i64,
        range_end_ms: i64,
    ) -> Result<BTreeMap<StatusRowKey, BTreeMap<i64, WorkspaceRollupCell>>, PersistenceError> {
        if row_keys.is_empty() {
            return Ok(BTreeMap::new());
        }
        let mut query = QueryBuilder::<Sqlite>::new(
        ...
        let rows = query.build().fetch_all(connection).await?;
        let mut grouped = BTreeMap::<StatusRowKey, BTreeMap<i64, WorkspaceRollupCell>>::new();
        for row in rows {
    ...
    pub(crate) async fn workspace_dirty_ranges(
        ...
        range_start_ms: i64,
        range_end_ms: i64,
    ) -> Result<BTreeMap<StatusRowKey, Vec<WorkspaceDirtyRange>>, PersistenceError> {
        if row_keys.is_empty() {
            return Ok(BTreeMap::new());
        }
        let mut query = QueryBuilder::<Sqlite>::new(
        ...
        let rows = query.build().fetch_all(connection).await?;
        let mut grouped = BTreeMap::<StatusRowKey, Vec<WorkspaceDirtyRange>>::new();
        for row in rows {
            grouped
                .entry((
                    row.get::<String, _>("monitor_id"),
    ...
    pub(crate) async fn list_executions_v2(
        &self,
        connection: &mut SqliteConnection,
    ...
    pub(crate) async fn execution_detail_v2(
        &self,
        connection: &mut SqliteConnection,
        ...
        (
            ...
            Vec<ChannelMonitorTargetResultRecord>,
        ),
        PersistenceError,
    > {
    ...
    pub(crate) async fn attempt_history_v2(
        &self,
        connection: &mut SqliteConnection,
...
fn endpoint_ping_from_row(row: &sqlx::sqlite::SqliteRow) -> Option<ChannelStatusEndpointPing> {
...
fn execution_summary_from_row(row: sqlx::sqlite::SqliteRow) -> ChannelMonitorExecutionSummaryV2 {
    ChannelMonitorExecutionSummaryV2 {
...
fn target_result_from_row(row: sqlx::sqlite::SqliteRow) -> ChannelMonitorTargetResultRecord {
...
fn attempt_from_row(row: sqlx::sqlite::SqliteRow) -> ChannelMonitorAttemptRecord {
