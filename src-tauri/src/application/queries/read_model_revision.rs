...
#[cfg_attr(
    ...
    expect(
        dead_code,
        reason = "contract=read-model.domain-revision-notice; owner=application/queries/read_model_revision; remove_when=all mutation facades emit typed revision notices"
    )
)]
