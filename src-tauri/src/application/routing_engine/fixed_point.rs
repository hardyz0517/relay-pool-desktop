...
impl BasisPoints {
    ...
    pub(crate) fn weighted_average(values: impl IntoIterator<Item = (Self, Self)>) -> Option<Self> {
        ...
        for (value, weight) in values {
            sum = sum.checked_add(u64::from(value.0) * u64::from(weight.0))?;
            weights = weights.checked_add(u64::from(weight.0))?;
        }
        if weights == 0 {
            return Some(Self::ZERO);
        }
        Self::new(u16::try_from((sum + weights / 2) / weights).ok()?)
    }
}
...
impl UtilityScore {
    pub(crate) fn new(value: BasisPoints) -> Self {
        Self(value)
    }
    pub(crate) fn value(self) -> BasisPoints {
        self.0
    }
}
#[cfg(test)]
...
impl FactorScore {
    pub(crate) fn effective(self) -> BasisPoints {
        self.value
            .checked_mul(self.confidence)
            .unwrap_or(BasisPoints::ZERO)
    }
}
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct FactorContribution {
    pub(crate) weight: BasisPoints,
    pub(crate) score: BasisPoints,
    pub(crate) contribution: BasisPoints,
}
#[cfg(test)]
mod tests {
    use super::*;
    #[test]
    fn fixed_point_is_bounded() {
        assert!(BasisPoints::new(10_001).is_none());
        assert!(BasisPoints::new(10_000)
            .unwrap()
            .checked_add(BasisPoints::new(1).unwrap())
            .is_none());
        assert_eq!(
            BasisPoints::weighted_average([
                (BasisPoints::new(1).unwrap(), BasisPoints::new(2).unwrap()),
                (BasisPoints::new(2).unwrap(), BasisPoints::new(1).unwrap())
            ])
            .unwrap()
            .get(),
            1
