...
pub(crate) struct ScoreInput {
    ...
    pub(crate) effective_capacity: u64,
    pub(crate) waiting: u64,
    pub(crate) error_rate_ewma: f64,
    pub(crate) ttft_ms: Option<f64>,
    pub(crate) quota_headroom: f64,
}
...
pub(crate) fn multiplier_factors(values: &[f64]) -> Vec<f64> {
    lower_is_better_factors(values, 1.0)
}

pub(crate) fn priority_factors(values: &[i64]) -> Vec<f64> {
    let values: Vec<_> = values.iter().map(|value| *value as f64).collect();
    lower_is_better_factors(&values, 1.0)
}

pub(crate) fn queue_factors(waiting: &[u64]) -> Vec<f64> {
...
pub(crate) fn error_factors(error_rate_ewmas: &[f64]) -> Vec<f64> {
    error_rate_ewmas
        .iter()
        .map(|value| {
            if value.is_finite() {
...
pub(crate) fn ttft_factors(ttfts: &[Option<f64>]) -> Vec<f64> {
...
pub(crate) fn load_rate(in_flight: u64, effective_capacity: u64) -> f64 {
