...
fn cleanup_unpublished_target(path: &Path) -> DataMigrationResult<()> {
    for candidate in [
        ...
        remove_file_if_exists_with_retry(&candidate)?;
    }
    Ok(())
}

fn remove_file_if_exists_with_retry(path: &Path) -> DataMigrationResult<()> {
    ...
    for attempt in 0..CLEANUP_ATTEMPTS {
        match fs::remove_file(path) {
            ...
            Err(_) if attempt + 1 < CLEANUP_ATTEMPTS => thread::sleep(CLEANUP_RETRY_DELAY),
            Err(_) => return Err(DataMigrationError::CleanupFailed),
        }
    }
    Ok(())
}
