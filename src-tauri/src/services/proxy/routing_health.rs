...
pub struct RouteHealthTransition {
...
pub fn failure_kind_code(kind: &RouteFailureKind) -> &'static str {
    match kind {
        RouteFailureKind::AuthError => "auth_error",
        RouteFailureKind::InsufficientBalance => "insufficient_balance",
        RouteFailureKind::RateLimited => "rate_limited",
        RouteFailureKind::ModelUnavailable => "model_unavailable",
        RouteFailureKind::CapabilityMismatch => "capability_mismatch",
        RouteFailureKind::BadRequest => "bad_request",
        RouteFailureKind::TemporaryNetwork => "temporary_network",
        RouteFailureKind::Upstream5xx => "upstream_5xx",
        RouteFailureKind::Timeout => "timeout",
        RouteFailureKind::StreamInterrupted => "stream_interrupted",
        RouteFailureKind::Unknown => "unknown",
    }
}

pub fn error_summary_for_failure(failure: &ClassifiedRouteFailure, message: &str) -> String {
    format!("{}: {}", failure_kind_code(&failure.kind), message)
}

pub fn error_summary_indicates_offline(summary: &str) -> bool {
...
mod tests {
    ...
    fn rate_limit_uses_retry_after_for_cooldown() {
        let failure = ClassifiedRouteFailure {
            kind: crate::services::proxy::routing_failure::RouteFailureKind::RateLimited,
            action: crate::services::proxy::routing_failure::RouteFailureAction::Cooldown,
            scope: crate::services::proxy::routing_failure::RouteFailureScope::KeyHealth,
