...
use crate::{
    ...
    services::{
        ...
        proxy::{
            ...
            redact_error_message, should_fallback, RouteCandidate,
        },
    },
};
...
fn handle_proxy_request(context: &ProxyServerContext, request: &ParsedRequest) -> ProxyResponse {
    match (request.method.as_str(), request.path.as_str()) {
        ...
        _ => ProxyResponse::json_error(
            404,
            "not_found",
            "Relay Pool Desktop P5 只支持 /v1/models 和 /v1/chat/completions。",
        ),
    }
}
...
fn forward_models_request(context: &ProxyServerContext, request: &ParsedRequest) -> ProxyResponse {
    ...
    forward_with_fallback(request, &candidates, None, false)
}
...
fn forward_chat_request(context: &ProxyServerContext, request: &ParsedRequest) -> ProxyResponse {
    ...
    forward_with_fallback(request, &candidates, model, stream)
}
...
fn forward_to_candidate(
    ...
    let url = build_upstream_url(&candidate.upstream_base_url, &request.path);
