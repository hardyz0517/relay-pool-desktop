...
pub(crate) struct V2RoutingRepository {
    ...
    data_key: [u8; 32],
    legacy_finalization: SqliteRoutingRepository,
}
...
impl V2RoutingRepository {
    pub(crate) fn new(
        routing: RoutingService,
        legacy_database: AppDatabase,
        data_key: [u8; 32],
    ) -> Self {
        Self {
            routing,
            data_key,
            legacy_finalization: SqliteRoutingRepository::new(legacy_database, data_key),
        }
    }
}
...
impl RoutingRepository for V2RoutingRepository {
    ...
    fn record_final_outcome(
        ...
        self.legacy_finalization.record_final_outcome(outcome)
    }
}
