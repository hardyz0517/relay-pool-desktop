...
async fn collect_balance(context: &CollectorContext<'_>) -> Result<DriverOutput, DriverFailure> {
    ...
    remove_unverified_usage_fields(&mut balance_data);
    let (today_usage, mut usage_evidence) =
        collect_today_dashboard_usage(context, &website_url, status.quota_per_unit).await;
    merge_today_usage_into_balance_data(&mut balance_data, today_usage);
...
async fn collect_today_dashboard_usage(
    context: &CollectorContext<'_>,
    website_url: &str,
    ...
    let stats = NewApiUsageStats {
        today_request_count: dashboard.request_count,
        today_consumption: dashboard.consumption,
        today_token_count: dashboard.token_count,
    };

    (stats.has_any().then_some(stats), evidence)
}
...
fn remove_unverified_usage_fields(data: &mut Value) {
...
fn merge_today_usage_into_balance_data(data: &mut Value, stats: Option<NewApiUsageStats>) {
...
mod tests {
    ...
    #[tokio::test]
    async fn newapi_balance_is_fixed_cost_and_does_not_collect_models_or_logs() {
        let server = TestHttpServer::sequence(vec![
            Some(json_response(
                200,
                json!({
    ...
    #[tokio::test]
    async fn optional_today_dashboard_failure_preserves_core_balance_fact() {
        let server = TestHttpServer::sequence(vec![
            Some(json_response(
                200,
                json!({
            ...
            Some(json_response(
                503,
                json!({"success": false, "message": "dashboard unavailable"}),
            )),
        ]);
        ...
        assert_eq!(requests.len(), 3);
        assert_eq!(balance.total_request_count, Some(1200));
        assert_eq!(balance.today_request_count, None);
        assert_eq!(balance.today_consumption, None);
        assert_eq!(balance.today_token_count, None);
    }

    #[tokio::test]
    async fn dashboard_window_only_reports_complete_non_negative_today_metrics() {
        let server = TestHttpServer::sequence(vec![Some(json_response(
            200,
            json!({
                "success": true,
                "data": [
                    { "count": 1, "quota": 200000, "token_used": 10 },
                    { "count": 2, "quota": 300000, "token_used": 20 }
                ]
            }),
        ))]);
        let outbound = AsyncOutboundClient::new(AsyncOutboundClientConfig::architecture_budget());
        ...
        assert_eq!(window.token_count, Some(30));
