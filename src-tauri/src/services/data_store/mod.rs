...
pub fn inspect_startup(default_data_dir: &Path) -> Result<DataStoreStartupState, String> {
    ...
    let generation = DatabaseGeneration::Two;
...
mod tests {
    ...
    fn startup_orchestration_discovers_without_mutation_or_trust_leaks() {
        ...
        let dir = temp_root("healthy-default");
        create_db(&dir);
        let state = inspect_startup(&dir).expect("healthy default");
        assert!(matches!(state.decision, StartupDecision::Ready { .. }));
        assert!(state.candidates.iter().any(|candidate| candidate.role == CandidateRole::Default));
    ...
    fn save_v2(dir: &Path, active_data_dir: Option<PathBuf>, pending_data_dir: Option<PathBuf>, source_data_dir: Option<PathBuf>) {
        write_config_v3(&config_path(dir), &DataDirConfigV3 {
            version: 3,
            active_data_dir,
            pending_data_dir,
            source_data_dir,
